...
class QB extends Player{ 
  ...
  update (){
    ...
    if(newGame){
      if(!this.getData("moving")){this.play("player");this.setData("moving",true);}
      if(this.x < this.getData("startX")){this.x += this.getData("speed");}
      if(this.x > this.getData("startX")){this.x -= this.getData("speed");}
      if(this.y < this.getData("startY")){this.y += this.getData("speed");}
    }
    if(ballSnapped){
      if(!this.getData("moving") && !this.getData("spinning") && !this.getData("blasting")){this.play("player");this.setData("moving",true);}
        if(offensiveTeam == 1){
...
class DB1 extends Player{
  constructor(scene, x, y, key, type) {
      super(scene, x, y, "player2", "db1");
      this.setData("speed", 1.2);
      this.setData("speedOriginal",1.2 + difficulty/2);
      this.scaleY = -1;
  }
  update (){
    if(this.x<0){this.x = 0;}if(this.x>640){this.x = 640;}if(this.y<0){this.y = 0;}if(this.y>1440){this.y = 1440;}
    if(this.getData("speed") < this.getData("speedOriginal")){this.setData("speed", this.getData("speed") + 0.1);}
    if(newGame){
      if(!this.getData("moving")){this.play("player2");this.setData("moving",true);}
      if(this.x < this.getData("startX")){this.x += this.getData("speed");}
      if(this.x > this.getData("startX")){this.x -= this.getData("speed");}
      if(this.y < this.getData("startY")){this.y += this.getData("speed");}
    }
...
class SS extends Player{
  ...
  update (){
    ...
    if(this.getData("hasBall")){
      ...
      if(team2Play == "slantX" || team2Play == "curlX"){
        this.y += this.getData("speed");
      }
    }
  }//end update
...
class LB2 extends Player{
  ...
  update (){
    ...
    if(kickOffStarted && kickReturnTeam == 1){
      this.y += 1;if(!this.getData("moving")){this.play("player2");this.setData("moving",true);}
