    ...
    */
    step:function(){
        ...
        while(body!==null){
            ...
            if(body.sleeping){
                if( body.angularVelocity.testZero() || body.linearVelocity.testZero() || body.position.testDiff(body.sleepPosition) || body.orientation.testDiff(body.sleepOrientation)){ body.awake(); } // awake the body
...
OIMO.RigidBody = function(X,Y,Z,Rad,Ax,Ay,Az){
	...
	//controllable compliance / penetration allowances
	
	this.compliance = 0.5
	
	this.allowablePen = 0.005
...
OIMO.RigidBody.prototype = {
    ...
    */
    setupMass:function(Type,AdjustPosition){
        ...
        if(adjustPosition){
            this.position.addEqual(tmpV); // the center of mass should move but the object itself shouldn't really ?.
...
OIMO.Body = function(Obj){
    ...
    if(obj.config){
        ...
        sc.friction = obj.config[1] === undefined ? 0.4 : obj.config[1]; // the old expression with || was not allowing 0 friction (obj.config[1] || 0.4;)
        // The coefficient of restitution of the shape.
        sc.restitution = obj.config[2] === undefined ? 0.2 : obj.config[2]; // the old expression with || was not allowing 0 friction (obj.config[2] || 0.2;)
    ...
    for(var i=0; i<type.length; i++){
        ...
        switch(type[i]){
            ...
            case "cylinder": shapes[i] = new OIMO.CylinderShape(sc, s[n+0], s[n+1]); break ;//new OIMO.BoxShape(sc, s[n+0], s[n+1], s[n+2]); break; // fake cylinder
...
OIMO.ContactConstraint.prototype.preSolve = function(timeStep,invTimeStep){
	...
	// use this to adjust how much compliance to penetration the solver has. If you set it at 0.3, it'll take around 3 steps (1 / 0.3) to fully "push out"
	// originally : -(p.penetration+0.005)*invTimeStep*0.05; 
	// this allows a 0.005 m penetration error, but really just ends up making the penetration sloppy AFAICT to prevent jitter on piles of objects,
	// but causes issue in that objects are compliant and never fully extract. 0.3 seems to be a good value to prevent jitter, but probably makes a stack unstable / jittery. 
	// If island size is considered it might be a good idea to dynamically scale the compliance to be soft with big piles of objects, and hard for strict pairs
	// it might also be good to store this on a per body basis
	// that way character controllers could interact cleanly with terrain. The value doesn't do anything with static terrain, so they could be flagged.
	// with the current state of affairs you can only choose compliance for all objects, or strict exit conditions for all objects, while character controllers vs. a pile of crates have very different needs.
	// characters need to not get stuck, but the crates need to pile in a stable way without issues/jitter. standing on crates might then become a problem but at least there would be control.
	
	var mixAllowablePen = (this.body1.allowablePen+ this.body2.allowablePen)/2
	var mixCompliance =  (this.body1.compliance+ this.body2.compliance)/2
	
	var sepV=-(p.penetration+mixAllowablePen)*invTimeStep*mixCompliance; 
    if(norTar<sepV)norTar=sepV; //-p.penetration*invTimeStep //
    c.norTar=norTar;
	//console.log(p.penetration)
	//if(p.penetration !== undefined) c.norTar = ;
    c.last=i==this.num-1;
    c=c.next;
    }
}
OIMO.ContactConstraint.prototype.solve = function(){
    ...
    while(true){
		...
		//add support for normally directed "down force" positive sucks body onto another, negative just makes it bounce strangely on contact. useful to makes stuff follow other bodies etc. at the cost of a single basic operation (?)
		// use average stickiness ? (two sticky objects are extra sticky together, whereas a non-sticky and sticky are only somewhat "sticky"), make neg values clamp to zero so you can have "super non-stick" surfaces
		newImp1 += (this.body1.stickiness+this.body2.stickiness)/(2);
...
OIMO.CylinderShape = function(config,radius,height){
    ...
    this.height=height;
    this.halfHeight=height*0.5;
    this.position.copy(config.relativePosition);
    this.rotation.copy(config.relativeRotation);
    this.friction=config.friction;
    this.restitution=config.restitution;
	...
	this.dimentions = new OIMO_ARRAY_TYPE(6);
...
OIMO.CylinderShape.prototype.calculateMassInfo = function(out){
    ...
    var inertiaY= this.radius*this.radius*mass; //originally loth wrote 1/2*this.radius*this.radius, is this false?
	out.inertia.init(
    inertiaXZ,0,0,
    0,inertiaY,0,
    0,0,inertiaXZ
...
OIMO.CylinderShape.prototype.updateProxy = function(){
   ...
   di[2] = dirZ;
   //half directions
   di[3] = dirX*this.halfHeight;
   di[4] = dirY*this.halfHeight;
   di[5] = dirZ*this.halfHeight;
...
/**
* An cylinder collision detectors
* @author lo-th(?)
*/
...
OIMO.BoxCylinderCollisionDetector.prototype.detectCollision = function(shape1,shape2,manifold){
    ...
    var ch=c.halfHeight;
    var r=c.radius;
    var nwx=b.dimentions[0];
    var nwy=b.dimentions[1];
    var nwz=b.dimentions[2];
    var nhx=b.dimentions[3];
    var nhy=b.dimentions[4];
    var nhz=b.dimentions[5];
    var ndx=b.dimentions[6];
    var ndy=b.dimentions[7];
    var ndz=b.dimentions[8];
    var dwx=b.dimentions[9];
    var dwy=b.dimentions[10];
    var dwz=b.dimentions[11];
    var dhx=b.dimentions[12];
    var dhy=b.dimentions[13];
    var dhz=b.dimentions[14];
    var ddx=b.dimentions[15];
    var ddy=b.dimentions[16];
    var ddz=b.dimentions[17];
    var ncx=c.dimentions[0];
    var ncy=c.dimentions[1];
    var ncz=c.dimentions[2];
    var dcx=c.dimentions[3];
    var dcy=c.dimentions[4];
    var dcz=c.dimentions[5];
    var nx=sep.x;
    var ny=sep.y;
    var nz=sep.z;
    ...
    var dir2l;
    if(state==0){
    manifold.addPoint(pos.x,pos.y,pos.z,nx,ny,nz,dep.x,this.flip);	//result.addContactInfo(pos.x,pos.y,pos.z,nx,ny,nz,dep.x,b,c,0,0,false);
    }else if(state==4){
    if(right4){
    ccx=pcx-dcx;
    ccy=pcy-dcy;
 ...
 //   v=b.vertex4;
  ...
  //  v=b.vertex2;
   ...
   // v=b.vertex1;
    ...
    if(pd<=0)manifold.addPoint(v1x,v1y,v1z,-nx,-ny,-nz,pd,this.flip);	//result.addContactInfo(v1x,v1y,v1z,-nx,-ny,-nz,pd,b,c,5,0,false);
    pd=nx*(v2x-ccx)+ny*(v2y-ccy)+nz*(v2z-ccz);
    if(pd<=0)manifold.addPoint(v2x,v2y,v2z,-nx,-ny,-nz,pd,this.flip);	//result.addContactInfo(v2x,v2y,v2z,-nx,-ny,-nz,pd,b,c,6,0,false);
    pd=nx*(v3x-ccx)+ny*(v3y-ccy)+nz*(v3z-ccz);
    if(pd<=0)manifold.addPoint(v3x,v3y,v3z,-nx,-ny,-nz,pd,this.flip);	//result.addContactInfo(v3x,v3y,v3z,-nx,-ny,-nz,pd,b,c,7,0,false);
    pd=nx*(v4x-ccx)+ny*(v4y-ccy)+nz*(v4z-ccz);
    if(pd<=0)manifold.addPoint(v4x,v4y,v4z,-nx,-ny,-nz,pd,this.flip);	//result.addContactInfo(v4x,v4y,v4z,-nx,-ny,-nz,pd,b,c,8,0,false);
    }else{
    switch(state){
    case 1:
    if(right1){
...
OIMO.CylinderCylinderCollisionDetector.prototype.getSep = function(c1, c2, sep, pos, dep) {
    ...
    while (true) {
        ...
        var hit = false;
        //iterations2 = 0 // there was no fail safe here, infinite loops could trigger if stuff went really wrong. not sure if this is the best protection but better than a permanent-hang. looks like this is a really bad bug tho
        while (true) {
            /*if (++iterations2 > 100) {
                console.log('maxed out in sep test, infinite loop prevented')
                return false;
            }*/
            t1x = v2x - v1x;
            t1y = v2y - v1y;
...
OIMO.CylinderCylinderCollisionDetector.prototype.detectCollision = function(shape1, shape2, manifold) { 
    ...
    var r1 = c1.radius;
    var r2 = c2.radius;
    var n1x = c1.dimentions[0];
    var n1y = c1.dimentions[1];
    var n1z = c1.dimentions[2];
    var n2x = c2.dimentions[0];
    var n2y = c2.dimentions[1];
    var n2z = c2.dimentions[2];

    var d1x = c1.dimentions[3];
    var d1y = c1.dimentions[4];
    var d1z = c1.dimentions[5];
    var d2x = c2.dimentions[3];
    var d2y = c2.dimentions[4];
    var d2z = c2.dimentions[5];
    var dx = p1x - p2x;
    var dy = p1y - p2y;
    var dz = p1z - p2z;
    var len;
    ...
    var pos = new OIMO.Vec3();
    var dep = new OIMO.Vec3();
    if (!this.getSep(c1, c2, sep, pos, dep)) return; // if no collision has occurred, getSep returns false and this function early outs, no contacts are generated. normally after the bug im running into, this just hangs cause garbage in -> endless loop, that means bug occurs in first iteration
...
OIMO.SphereCylinderCollisionDetector.prototype.detectCollision = function(shape1,shape2,manifold){
    ...
    var pcy=pc.y;
    var pcz=pc.z;
    var dirx=c.dimentions[0];
    var diry=c.dimentions[1];
    var dirz=c.dimentions[2];
    var rads=s.radius;
    var radc=c.radius;
    var rad2=rads+radc;
    ...
    manifold.addPoint(psx+dx*rads,psy+dy*rads,psz+dz*rads,dx,dy,dz,len-rads,this.flip); // for some reason normal needs to be flipped here, so pass a true. this seems to be working now
	}
}
/**
